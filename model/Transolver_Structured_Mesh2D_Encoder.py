...
from model.Physics_Attention import Physics_Attention_Structured_Mesh_2D_Auto_Encoder, My_Physics_Attention
...
class Transolver_Encoder_block(nn.Module):
    ...
    def __init__(
        ...
        self.Attn = Physics_Attention_Structured_Mesh_2D_Auto_Encoder(hidden_dim, heads=num_heads, dim_head=hidden_dim // num_heads,
    ...
    def encode(self, fx):
        if self.last_layer:
            return self.Attn.encode(self.ln_1(fx), cache_slice=True)
    ...
    def decode(self, code):
        #we should not use fx in decoder
        if not(self.last_layer):
            print("the model has to be the last layer")
            return
        
        fx = self.Attn.reconstruct_fx(code)
        fx = self.Attn.decode(code) + fx
        fx = self.mlp(self.ln_2(fx)) + fx
...
class Model(nn.Module):
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([Transolver_Encoder_block(num_heads=n_head, hidden_dim=n_hidden,
                                                      dropout=dropout,
                                                      act=act,
                                                      mlp_ratio=mlp_ratio,
                                                      out_dim=out_dim,
    ...
    def decode(self, code):
        return self.blocks[-1].decode(code)
