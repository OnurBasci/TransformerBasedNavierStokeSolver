...
def main():
    ...
    if eval:
        ...
        for ep in range(args.epochs):
            ...
            if ep%10 == 0 and ep >= 10 and look_ahead <= max_look_ahead:
                look_ahead += 1
                offset = step * look_ahead
                model.n = look_ahead
            ...
            for i, (x, fx, yy) in enumerate(train_loader):
                ...
                for t in range(0, T-offset+step, step):
                    y = yy[..., t+offset-step: t+offset]
                    im = model(x, fx)
                    loss += myloss(im.reshape(bsz,-1), y.reshape(bsz, -1))
                    #concatenate fx with the next velocity field and not the prediction
                    next_y = yy[..., t:t+step]
                    fx = torch.cat((fx[..., step:], next_y), dim=-1)

                train_l2_step += loss.item()
                #the backpropagation is done after the simulation is looped
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            print(
                "Epoch {} , train_step_loss:{:.5f}".format(ep, train_l2_step))
